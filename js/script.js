    ...
    const products = [
      {
        ...
        size: "24 × 36 in",
        available: true,
        inventoryNote: "One-of-one original painting.",
      ...
      },
      {
        ...
        size: "Select size",
        available: true,
        inventoryNote: "Printed after order confirmation.",
    ...
    const DISCOUNT_CODES = {
      LOCAL10: {
        rate: 0.10,
        reason: "10% local neighborhood discount"
      },
      ARTIST10: {
        rate: 0.10,
        reason: "10% artist community discount"
      },
      LESTER15: {
        rate: 0.15,
        reason: "15% special Lester print discount"
      }
    };
    ...
    function getDiscount(product) {
      ...
      if (product.type !== "Fine Art Print") return { rate: 0, reason: "Discounts apply to prints only." };
      if (isLocal) return { rate: 0.15, reason: "15% neighborhood local print discount." };
      if (isArtist) return { rate: 0.10, reason: "10% artist community print discount." };
      ...
      home: () => `
        <div class="canvas-section hero-grid">
          <div>
            ...
            <p class="section-copy">A premium interactive art site built like a studio table: one palette, one canvas, clear paths for collectors, print buyers, and artists who want tools worth using.</p>
          ...
          <div class="hero-art" aria-label="Featured abstract painting"><div class="hero-art-label">Currently Mixing<br>Golden Hour Serenity</div></div>
        </div>
      `,
      gallery: () => `
        <div class="canvas-section">
          <h2 class="section-heading">A gallery with <span class="paint-script">presence.</span></h2>
          <p class="section-copy">Focused and dramatic. Replace these art panels with Lester’s real painting photos when ready.</p>
          <div class="gallery-grid">
            <article class="art-tile big" style="--bg-art: linear-gradient(135deg,#e63946,#fcbf49,#f77f00);"><strong>Sunburst Alley</strong></article>
            <div class="gallery-side">
              <article class="art-tile" style="--bg-art: linear-gradient(135deg,#277da1,#7b2cbf);"><strong>Blue Hour Dreams</strong></article>
              <article class="art-tile" style="--bg-art: linear-gradient(135deg,#2a9d8f,#277da1,#fcbf49);"><strong>Garden Wall Rhythm</strong></article>
            </div>
          </div>
        </div>
      `,
    shop: () => `
      <div class="canvas-section">
        <h2 class="section-heading">Paintings & <span class="paint-script">Prints</span></h2>
        <p class="section-copy">
          Originals ship from Lester. Prints are ordered from the print company after confirmation.
          Customers can request shipping or local pickup.
        </p>
        
        <div class="shop-grid" id="shopGrid"></div>
        
        <form class="order-form premium-order-form" id="orderForm">
          <h3 style="margin:0;">Order Sheet</h3>
        
          <label for="productSelect">Selected artwork</label>
          <select id="productSelect" required></select>
        
          <div id="printOptionsWrap" class="hidden-field">
          ...
          <input id="discountCode" placeholder="LOCAL10, ARTIST10, etc." />
    ...
    `,
      tools: () => `
        <div class="canvas-section">
          ...
          <p class="section-copy">Interactive tools give artists a reason to come back — palette generation, ratios, sketching, and creative prompts.</p>
      ...
      about: () => `
        <div class="canvas-section">
          ...
          <div class="feature-row">
            <div class="feature-card"><strong>Studio Energy</strong><p class="section-copy">Colorful, welcoming, expressive, and hands-on.</p></div>
            <div class="feature-card"><strong>Collector Friendly</strong><p class="section-copy">Clear fulfillment notes for originals and prints.</p></div>
            <div class="feature-card"><strong>Artist Useful</strong><p class="section-copy">Tools and prompts make the site worth returning to.</p></div>
          </div>
        </div>
      `,
      socials: () => `
    ...
    function initShop() {
      ...
      shopGrid.innerHTML = products.map(product => `
        <article class="product-card">
          <div class="product-art" style="--bg-art:${product.bg};"></div>
          <div class="product-body">
            <h3>${product.title}</h3>
            <p class="section-copy">${product.description}</p>
            <div class="product-meta"><span>${product.size}</span><span class="price">${money(product.price)}</span></div>
            <p class="section-copy"><strong>${product.type}</strong><br>${product.fulfillment}</p>
            <button class="btn btn-primary btn-small" data-product="${product.id}" type="button">Select Piece</button>
          </div>
        </article>
      `).join("");

      productSelect.innerHTML = products.map(product => `<option value="${product.id}">${product.title} — ${product.type} — ${money(product.price)}</option>`).join("");
    ...
    function updateTotal() {
      ...
      $("#totalBox").innerHTML = `<div>Selected: ${product.title}</div><div>Base price: ${money(product.price)}</div><div>Discount: ${money(discountAmount)} — ${discount.reason}</div><div>Estimated subtotal before shipping/tax: ${money(subtotal)}</div>`;
    }
    ...
    function generatePalette() {
      ...
      $("#paletteOutput").innerHTML = colors.map(color => `<button class="swatch" title="Copy ${color}" style="background:${color}" onclick="navigator.clipboard?.writeText('${color}')" type="button"></button>`).join("");
    }
    ...
    function initSketchCanvas() {
      ...
      $("#thinBrush").addEventListener("click", () => brushSize = 4);
      $("#thickBrush").addEventListener("click", () => brushSize = 14);
